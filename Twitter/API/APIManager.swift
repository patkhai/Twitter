...
class APIManager: SessionManager {
    ...
    static let consumerKey = "jfqoZx29Rx1mYMaNNOLSLjJ12"
    static let consumerSecret = "cev1Gmrnmn4HLeVbuvsKWwPd9jjTrIm5kK3WicdTbJSrMjCGXk"
    ...
    func unRetweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let baseURLString = " https://api.twitter.com/1.1/statuses/unretweet/"
        let parameters = ["id": tweet.id]
        let urlString = baseURLString + "\(tweet.id).json"
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
